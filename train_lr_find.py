# encoding:utf-8
import os
import tensorflow.compat.v1 as tf  # tensorflow 2.x 兼容 1.x
import numpy as np
...
tf.set_random_seed(2018)  # tf.random.set_seed(2018)  
np.random.seed(2018)
...
def main(_):
    ...
    (x_train, y_train), (x_test, y_test) = mnist.load_data()  # 默认下载或加载
    ...
    lr_finder.find(trainDataset=train_dataset, start_lr=1e-5, end_lr=10, optimizer=optimizer, epochs=1)
